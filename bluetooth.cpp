 ...
 */
class SmartLockProcess : public BLEProcess {
public:
  SmartLockProcess(events::EventQueue &event_queue, BLE &ble_interface)
...
class BLEPasswordListener : ble::GattServer::EventHandler {
  ...
  // Was: 0xA000
  const static uint16_t EXAMPLE_SERVICE_UUID = 0x1812; // Human Interface Device
...
public:
  BLEPasswordListener() {
    ...
    const UUID uuid = WRITABLE_CHARACTERISTIC_UUID;
    _writable_characteristic =
        new WriteOnlyGattCharacteristic<uint8_t>(uuid, _characteristic_value);
...
int init_bluetooth(EventQueue event_queue) {
  ...
  SmartLockProcess ble_process(event_queue, ble);
